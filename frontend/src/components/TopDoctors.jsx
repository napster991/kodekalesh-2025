...
const TopDoctors = () => {
// Instead of importing we can get doctors from context since we added it to value in context provider
    ...
    <div className='flex flex-col justify-center items-center gap-4 my-16'>
        ...
        <div className='w-full grid grid-cols-5 gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
            {doctors.slice(0,10).map((item,idx)=>(
                <div onClick={()=>{navigate(`/appointment/${item._id}`);scrollTo(0,0)}} key={idx} className='border-blue-200 border rounded-xl overflow-hidden cursor-pointer hover:-translate-y-2.5 transition-all duration-500'>
